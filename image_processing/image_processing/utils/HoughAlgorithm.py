...
from image_processing.utils.Lines import *
...
class HoughAlgorithm(Algorithm):
    ...
    def process_frame(self, frame, show=True):
        ...
        if show:
            line_img = draw_lines_on_frame(lines, frame.copy())
        ...
        if show:
            vanishing_point = draw_vanishing_point(line_img, x_points, y_points, show)
        else:
            vanishing_point = draw_vanishing_point(frame, x_points, y_points, show)

        # Calculating angle from vanishing point to (self.WIDTH // 2, 0)
        angle = calculate_angle_from_v_point(vanishing_point, self.WIDTH, self.HEIGHT)
